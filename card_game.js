...
const guess = (card1, card2) => {
  ...
  if (input === "h") {
    ...
    getInput('Sorry, no points this round. You must use only the leters "h" or "l".');
    return false;
  }
}
...
const playGame = () => {
  ...
  while (score < 5 && score < deck.length) {
    ...
    if (guess(currentCard, nextCard)) {
      score = score + 1;
      console.log(`Hooray ${playerName}! You got it correct. Your score is now ${score}.`)
    } else {
      console.log('Sorry, you were wrong. You get no points.')
    }
    currentCard = nextCard;
  }
  deck.length === 0 ? console.log("Sorry, you lose.  You are out of cards.") : console.log(`Hooray ${playerName}!  You have won!`)
